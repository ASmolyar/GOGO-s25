...
const StyledMapContainer = styled(MapContainer)<{ isRegionSelected: boolean; regionColor?: string }>`
  height: 0;
  width: 100%;
  ...
  .leaflet-container {
    ...
    width: 100%;
    height: 100%;
    cursor: ${props => props.isRegionSelected ? 'grab' : 'default'} !important;
...
const calculateRegionBounds = (region: Region): L.LatLngBounds => {
  ...
  const padRatio = Math.max(0.2, Math.min(0.5, 0.5 - (areaSizeKm / 200000) * 0.3));
...
const calculateOptimalView = (region: Region): {
  ...
  const visualPadRatio = Math.max(0.05, Math.min(0.15, 0.15 - (latSpread * lngSpread * 111 * 111 / 100000) * 0.1));
  
  // Visual bounds for indicators
  const visualBounds = L.latLngBounds(
    L.latLng(bounds.getSouth() - latSpread * visualPadRatio, bounds.getWest() - lngSpread * visualPadRatio),
    L.latLng(bounds.getNorth() + latSpread * visualPadRatio, bounds.getEast() + lngSpread * visualPadRatio)
  );
  ...
  const zoom = region.defaultZoom ? Math.max(calculatedZoom, region.defaultZoom) : calculatedZoom;
  
  return { center, zoom, minZoom, bounds, visualBounds };
};

// Pre-calculate and enhance region data with optimal view parameters
const enhanceRegionData = (region: Region): Region & {
  optimalView: {
    center: [number, number];
    zoom: number;
    minZoom: number;
...
function MapController({ 
  ...
  const entryTimeRef = useRef<number>(0); // Track when we entered a region
  
  // Calculate bounds dynamically based on region data
  const viewSettings = useMemo(() => {
    if (region) {
      // Use pre-calculated optimal view if available
      ...
      // Otherwise, calculate bounds based on sublocation spread
      const bounds = calculateRegionBounds(region);
      
      // Create a mock map to calculate the proper zoom level
      const mockMapDiv = document.createElement('div');
      mockMapDiv.style.width = '1000px';
      mockMapDiv.style.height = '600px';
      const mockMap = L.map(mockMapDiv, { doubleClickZoom: false });
      mockMap.fitBounds(bounds);
      const calculatedZoom = mockMap.getZoom();
      mockMap.remove();
      
      // Allow at most 1 zoom level out from optimal view
      const minZoom = Math.max(calculatedZoom - 1, 9);
      
      return {
        center: [bounds.getCenter().lat, bounds.getCenter().lng] as [number, number],
        zoom: Math.max(calculatedZoom, region.defaultZoom || calculatedZoom),
        minZoom: minZoom,
        bounds,
        visualBounds: bounds
      };
    ...
    // Use default bounds for USA overview
    return {
      center: lastRegionCoordinates || USA_MAP_CENTER,
      zoom: USA_DEFAULT_ZOOM,
      minZoom: USA_MIN_ZOOM,
      bounds: defaultMaxBounds ? L.latLngBounds(defaultMaxBounds[0], defaultMaxBounds[1]) : undefined,
      visualBounds: defaultMaxBounds ? L.latLngBounds(defaultMaxBounds[0], defaultMaxBounds[1]) : undefined
    };
  }, [region, defaultMaxBounds, lastRegionCoordinates]);
  
  // Reset view and adjust to region bounds whenever region changes
  useEffect(() => {
    // Handling the initial view and bounds
    const initializeMap = () => {
      // Reset any existing inertia when changing regions
      map.stop();
      
      if (region) {
        console.log(`Setting up view for region: ${region.name}`);
        
        // Record the time of region entry
        entryTimeRef.current = Date.now();
        
        // Set the view with animation if this isn't the initial load
        map.setView(viewSettings.center, viewSettings.zoom, {
          animate: true,
          duration: 1.0 // 1 second animation
        });
        
        // Set zoom constraints to keep view tight around locations
        // Never allow zooming out more than 1 level from optimal view
        map.setMinZoom(viewSettings.minZoom);
        map.setMaxZoom(16); // Allow zooming in for detailed views
        
        // Set max bounds to prevent dragging too far outside the region
        // But keep it very loose to allow good panning freedom
        if (viewSettings.bounds) {
          // Add very large padding to the max bounds
          const latSpread = viewSettings.bounds.getNorth() - viewSettings.bounds.getSouth();
          const lngSpread = viewSettings.bounds.getEast() - viewSettings.bounds.getWest();
          
          // Create much larger bounds for the maxBounds (100% padding)
          // This just prevents the user from panning extremely far away
          const maxBounds = L.latLngBounds(
            L.latLng(viewSettings.bounds.getSouth() - latSpread * 1.0, viewSettings.bounds.getWest() - lngSpread * 1.0),
            L.latLng(viewSettings.bounds.getNorth() + latSpread * 1.0, viewSettings.bounds.getEast() + lngSpread * 1.0)
          );
          
          map.setMaxBounds(maxBounds);
          console.log(`Set generous max bounds for ${region.name} with large padding`);
        }
        
        console.log(`Set zoom constraints for ${region.name}: min=${viewSettings.minZoom}, max=16`);
      } else {
        // For USA overview, center on last viewed region if available
        console.log(`Setting USA overview map view ${lastRegionCoordinates ? 'centered on last region' : ''}`);
        ...
        // Set USA bounds and zoom constraints
    ...
    // Set up interactions based on whether a region is selected
    if (region) {
      // Enable interactions for region view
      ...
      // Disable touch zoom for mobile to avoid issues
      map.touchZoom.disable();
      
      // Track if we're already adjusting the map to prevent recursive calls
      let isAdjusting = false;
      
      // Handler to keep the map within bounds
      const moveendHandler = () => {
        if (isAdjusting || !viewSettings.bounds) return;
        
        const center = map.getCenter();
        
        // Only adjust if we're significantly out of bounds (creates a soft boundary)
        // This allows some panning freedom beyond the actual content
        if (!viewSettings.bounds.contains(center)) {
          console.log("Map significantly out of bounds, adjusting...");
          
          // Calculate how far outside the bounds we are
          const south = viewSettings.bounds.getSouth();
          const north = viewSettings.bounds.getNorth();
          const west = viewSettings.bounds.getWest();
          const east = viewSettings.bounds.getEast();
          
          // Calculate distances beyond bounds
          const beyondSouth = Math.max(0, south - center.lat);
          const beyondNorth = Math.max(0, center.lat - north);
          const beyondWest = Math.max(0, west - center.lng);
          const beyondEast = Math.max(0, center.lng - east);
          
          // Define a threshold - how far beyond bounds we allow before adjusting
          // This creates a "soft boundary" effect
          const threshold = {
            lat: (north - south) * 0.1, // 10% of the latitude range
            lng: (east - west) * 0.1    // 10% of the longitude range
          };
          
          // Only adjust if we've gone beyond the threshold
          const needsAdjustment = 
            beyondSouth > threshold.lat ||
            beyondNorth > threshold.lat ||
            beyondWest > threshold.lng ||
            beyondEast > threshold.lng;
          
          if (needsAdjustment) {
            // Calculate constrained coordinates with a very small buffer
            const newLat = Math.max(
              south + 0.0001, 
              Math.min(north - 0.0001, center.lat)
            );
            const newLng = Math.max(
              west + 0.0001, 
              Math.min(east - 0.0001, center.lng)
            );
            
            const newCenter = new L.LatLng(newLat, newLng);
            
            isAdjusting = true;
            map.panTo(newCenter, { animate: true, duration: 0.5 }); // Smoother animation
      ...
      // Handler to detect when user tries to zoom out too far
      const zoomEndHandler = () => {
        const currentZoom = map.getZoom();
        const minZoom = map.getMinZoom();
        const timeSinceEntry = Date.now() - entryTimeRef.current;
        
        // Only allow auto-exit if we've been in the region for at least 1 second
        // This prevents accidental exits right after entering a region
        if (currentZoom === minZoom && !isTransitioning.current && timeSinceEntry > 1000) {
          console.log(`Reached min zoom (${minZoom}), auto-returning to USA map`);
          ...
          setTimeout(() => {
            ...
            }, 1100); // Just after animation completes
          }, 50);
        }
      };
      ...
      return () => {
        ...
        // Re-enable standard interactions
        map.dragging.enable();
      ...
      // USA Overview mode - disable all interactions to make it a fixed map
      ...
      // Cleanup function
      return () => {
        // Re-enable interactions when switching to region view
        map.dragging.enable();
        map.touchZoom.enable();
        map.doubleClickZoom.enable();
        map.scrollWheelZoom.enable();
  ...
  }, [map, region, viewSettings, onExitRegion, lastRegionCoordinates]);
  
  // We don't need to render anything in this controller component
  return null;
}
...
const PopupTitle = styled.h3<{ color?: string }>`
  ...
  &::after {
    ...
    width: 40%;
    height: 2px;
    background: linear-gradient(90deg, ${props => props.color || COLORS.gogo_blue}, rgba(107, 114, 253, 0));
  }
`;
...
const PopupType = styled.div<{ color?: string }>`
  ...
  background: linear-gradient(90deg, ${props => props.color ? `${props.color}40` : `${COLORS.gogo_purple}40`}, ${props => props.color ? `${props.color}20` : `${COLORS.gogo_blue}20`});
...
const SupportedBy = styled.div`
  ...
  margin-top: 16px;
  color: ${COLORS.gogo_yellow};
  background: linear-gradient(90deg, rgba(30, 30, 36, 0.4), rgba(40, 40, 48, 0.4));
  padding: 8px 12px;
  border-radius: 6px;
...
const ViewRegionButton = styled.button<{ color?: string }>`
  margin-top: 14px;
  padding: 8px 16px;
  background: linear-gradient(90deg, ${props => props.color || COLORS.gogo_blue}, ${COLORS.gogo_purple});
  color: white;
  border: none;
  border-radius: 6px;
  cursor: pointer;
...
function MapCustomControls({ onExitRegion, isRegionSelected }: MapCustomControlsProps) {
  return (
    <MapControls isVisible={isRegionSelected}>
      <ControlButton
        onClick={onExitRegion}
        title="Return to USA map"
      >
        <USAIcon />
      </ControlButton>
    </MapControls>
...
const BoundaryIndicator = styled.div<{ visible: boolean; direction: 'top' | 'right' | 'bottom' | 'left' }>`
  position: absolute;
  z-index: 1000;
  pointer-events: none;
...
function MapStats({ regions, selectedRegion }: MapStatsProps) {
  ...
  const statsData = useMemo(() => {
    if (selectedRegion) {
      ...
      const cityRegions = regions.filter(region => region.id !== 'summer-programs');
      const cityCount = cityRegions.length;
      const locationCount = regions.reduce(
        (count, region) => count + region.sublocations.length, 
        0
      );
      const programCount = regions.reduce((count, region) => {
        const programsInRegion = region.sublocations.reduce(
          (programCount, location) => programCount + (location.programs?.length || 0), 
          0
        );
        return count + programsInRegion;
      }, 0);
...
const NavigationInstructions = styled.div<{ isVisible: boolean; isRegionView: boolean; regionColor?: string }>`
  position: absolute;
  ${props => props.isRegionView 
    ? `
      bottom: 15px;
      left: 15px;
      top: auto;
    ...
    : `
      top: 15px;
      left: 50%;
      transform: translateX(-50%) translateY(${props.isVisible ? '0' : '-20px'});
      text-align: center;
    `
  }
  background: rgba(20, 20, 28, 0.85);
  ...
  border: 1px solid ${props => {
    if (props.isRegionView) return props.regionColor ? `${props.regionColor}80` : `${COLORS.gogo_yellow}80`;
...
function MapNavigationInstructions({ selectedRegion, showInstructions }: MapNavigationInstructionsProps) {
  const isRegionView = !!selectedRegion;
  
  return (
    <NavigationInstructions isVisible={showInstructions} isRegionView={isRegionView}>
      <InfoIcon isRegionView={isRegionView}>{isRegionView ? "🔍" : "i"}</InfoIcon>
      <div>
        {isRegionView
...
function BoundaryIndicators({ map, bounds, isRegionSelected }: BoundaryIndicatorsProps) {
  ...
  useEffect(() => {
    ...
    const updateIndicators = () => {
      ...
      const nearBoundary = {
        top: center.lat > north - threshold.lat, // Near north boundary
        right: center.lng > east - threshold.lng, // Near east boundary
        bottom: center.lat < south + threshold.lat, // Near south boundary
        left: center.lng < west + threshold.lng  // Near west boundary
      };
      ...
      const atBoundary = 
        (center.lat >= north - threshold.lat / 3) || 
        (center.lng >= east - threshold.lng / 3) ||
        (center.lat <= south + threshold.lat / 3) ||
        (center.lng <= west + threshold.lng / 3);
  ...
  return (
    <>
      ...
      <BoundaryMessage visible={showMessage}>
        You've reached the <strong>edge of this region</strong>. Zoom out to see other regions.
      </BoundaryMessage>
    </>
  );
}
...
function MapEvents({ region }: { region: Region & { optimalView?: any } }) {
  ...
  return (
    <BoundaryIndicators 
      map={map} 
      bounds={region.optimalView?.visualBounds || region.optimalView?.bounds} 
...
function Map({
  ...
  }) | undefined>(enhancedActiveRegion);
  
  // Keep track of the last selected region's coordinates for centering the USA map when zooming out
  const [lastRegionCoordinates, setLastRegionCoordinates] = useState<[number, number] | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (enhancedActiveRegion?.id !== previousRegionId) {
      console.log(`Region changed from ${previousRegionId} to ${enhancedActiveRegion?.id}`);
  ...
  return (
    <StyledMapContainer
      key={`map-${selectedRegion?.id || 'overview'}-${mapCenter[0]}-${mapCenter[1]}`}
      center={mapCenter}
      zoom={mapZoom}
      className={className}
      zoomControl={false}
    ...
    >
      <TileLayer
        ...
        keepBuffer={4} // Reduced from 8 to 4 to prevent triggering exit behavior
        updateWhenIdle={true} // Changed to true to prevent rapid updates during transitions
        updateWhenZooming={false} // Keep this for smooth zooming
      />
      
      {/* Add region markers if no specific region is selected */}
      {!selectedRegion && filteredRegions.map((region) => (
        <Marker
          key={region.id}
          position={region.centerCoordinates}
          icon={createRegionIcon(region.color || COLORS.gogo_blue, region.name)}
          eventHandlers={{
        ...
        >
          ...
          <Popup>
            <PopupContent>
              ...
              <div style={{ fontSize: '14px', marginBottom: '10px' }}>{region.sublocations.length} locations</div>
              {region.description && (
                <PopupDescription>{region.description}</PopupDescription>
              )}
              <ViewRegionButton color={region.color} onClick={() => handleRegionSelect(region)}>
                Explore {region.name}
              </ViewRegionButton>
            </PopupContent>
          </Popup>
      ...
      {selectedRegion && selectedRegion.sublocations.map((location) => (
        <Marker
          key={location.id}
          position={location.coordinates}
          icon={createMarkerIcon(location.type || 'default', selectedRegion.color)}
          eventHandlers={{
        ...
        >
          <Popup>
            <PopupContent>
              ...
              {location.type && <PopupType>{location.type.replace('-', ' ')}</PopupType>}
      ...
      {/* Boundary indicators */}
      {selectedRegion && (
        <MapEvents region={selectedRegion} />
      )}
      
      {/* Instruction tooltip - show only in region view */}
      <MapInstruction visible={selectedRegion !== undefined}>
        Click and <span>drag</span> to pan around the map • Use <span>scroll wheel</span> to zoom
      </MapInstruction>
      
      {/* Stats components - always show */}
      {showStats && <MapStats regions={regions} selectedRegion={selectedRegion} />}
      
      {/* Navigation instructions - show different content based on context */}
      {showInstructions && (
        <MapNavigationInstructions 
